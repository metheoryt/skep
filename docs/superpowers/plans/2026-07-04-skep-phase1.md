# skep Phase 1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A per-machine supervisor daemon (`skep`) that dispatches headless Claude Code agents from Telegram, streams their live activity into a per-task forum topic, and lets the owner list/kill them — with a hard owner-ID auth lock and an audit log.
...
**Files:**
- Create: `pyproject.toml`, `.gitignore`, `README.md`, `src/skep/__init__.py`, `src/skep/config.py`
...
- [ ] **Step 4: Create `src/skep/__init__.py`**
...
from skep.config import Config, load_config
...
def _base_env():
    return {
        ...
        "SKEP_WORKTREES_ROOT": "/home/me/.skep/worktrees",
    }
...
def test_load_config_parses_all_fields():
    ...
    assert cfg == Config(
        ...
        worktrees_root=Path("/home/me/.skep/worktrees"),
        claude_bin="claude",
    )


def test_load_config_claude_bin_override():
    env = _base_env() | {"SKEP_CLAUDE_BIN": "/opt/claude"}
...
Expected: FAIL — `ModuleNotFoundError: No module named 'skep.config'`

- [ ] **Step 7: Write `src/skep/config.py`**
...
```bash
git add pyproject.toml .gitignore README.md src/skep/__init__.py src/skep/config.py tests/test_config.py
...
Expected: FAIL — `ModuleNotFoundError: No module named 'skep.db'`

- [ ] **Step 3: Write `src/skep/db.py`**
...
Expected: FAIL — `ModuleNotFoundError: No module named 'skep.stream'`

- [ ] **Step 3: Write `src/skep/stream.py`**
...
Expected: FAIL — `ModuleNotFoundError: No module named 'skep.formatting'`

- [ ] **Step 3: Write `src/skep/formatting.py`**
...
**Files:**
- Create: `src/skep/agent.py`, `tests/conftest.py`, `tests/fixtures/fake_claude.py`
- Test: `tests/test_agent.py`

**Interfaces:**
- Consumes: `Event`, `parse_event` from `skep.stream`.
- Produces:
...
Expected: FAIL — `ModuleNotFoundError: No module named 'skep.agent'`

- [ ] **Step 5: Write `src/skep/agent.py`**
...
```bash
git add src/skep/agent.py tests/test_agent.py tests/conftest.py tests/fixtures/fake_claude.py
...
**Files:**
- Create: `src/skep/telegram_gw.py`
- Test: `tests/test_telegram_gw.py`

**Interfaces:**
- Consumes: `Config` from `skep.config`.
- Produces:
...
from skep.config import Config
...
Expected: FAIL — `ModuleNotFoundError: No module named 'skep.telegram_gw'`

- [ ] **Step 3: Write `src/skep/telegram_gw.py`**
...
```bash
git add src/skep/telegram_gw.py tests/test_telegram_gw.py
...
Expected: FAIL — `ModuleNotFoundError: No module named 'skep.supervisor'`

- [ ] **Step 3: Write `src/skep/supervisor.py`**
...
class Supervisor:
    ...
    async def spawn(self, repo: str, task_text: str) -> int:
        ...
        branch = f"skep/{_slug(task_text)}-{tid}"
...
- Produces:
  ...
  - `def run() -> None` — `asyncio.run(main())` (the `skep` console-script entry point).
...
from skep.app import format_ls
from skep.config import Config
from skep.db import Registry
from skep.supervisor import Supervisor
...
Expected: FAIL — `ModuleNotFoundError: No module named 'skep.app'`

- [ ] **Step 3: Write `src/skep/app.py`**
...
async def main() -> None:
    ...
    registry = Registry.open(os.environ.get("SKEP_DB", "skep.sqlite"))
...
- `skep` core, asyncio ✔ (app.py) · SQLite registry ✔ (Task 2) · survives-restart via session_id persistence ✔ (registry stores `session_id`; resume-*on-restart* wiring is Phase 3, spec §7).
