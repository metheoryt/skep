...
class MemoryStore:
    ...
    def _load(self, repo_path: Path) -> list[MemoryFact]:
        ...
        Logs AT MOST ONCE per call (spec §6): `Path.glob()` silently swallows a
        PermissionError/NotADirectoryError and yields nothing, so use
        `list(d.iterdir())` — which raises — to detect an unreadable or
        non-directory store; and aggregate per-file skips into one summary line
        so N corrupt files do not emit N warnings on every spawn.
        """
        d = memory_dir(repo_path)
        try:
            paths = sorted(p for p in d.iterdir() if p.suffix == ".md")
        except FileNotFoundError:
            return []  # absent store is the normal case; not worth a log line
...
#
# A traversal string that slugify NEUTRALIZES proves nothing about the
# containment boundary — slugify strips it to a safe name before the boundary
# ever runs. So the cases split by what they exercise:
#   NEUTRALIZED: slugify turns them into a safe, contained slug -> the write
#                SUCCEEDS and lands inside .agent-memory/. Assert containment.
#   REJECTED:    slugify produces an empty slug and raises -> the write is
#                refused. Assert the raise.
# `supersedes` differs: ANY of these values names no existing file (the
# neutralized slug does not exist either), so supersedes rejects all of them.

NEUTRALIZED = ["../escape", "../../../.ssh/authorized_keys", "/etc/passwd"]
REJECTED = ["!!!", ""]


@pytest.mark.parametrize("evil", NEUTRALIZED)
def test_title_traversal_is_neutralized_and_contained(tmp_path, evil):
    path = write_memory(tmp_path, evil, "b", now=_clock())
    assert path.resolve().parent == (tmp_path / ".agent-memory").resolve()
    assert not (tmp_path.parent / ".ssh").exists()
    assert not Path("/etc/passwd").is_symlink()  # sanity: nothing pointed at it


@pytest.mark.parametrize("evil", REJECTED)
def test_title_that_slugifies_empty_is_rejected(tmp_path, evil):
    with pytest.raises(ValueError):
        write_memory(tmp_path, evil, "b", now=_clock())


@pytest.mark.parametrize("evil", NEUTRALIZED + REJECTED)
def test_supersedes_traversal_rejected(tmp_path, evil):
...
def test_title_collision_suffix_does_not_escape_via_symlink(tmp_path):
    # The base slug is legitimately occupied, so _next_free tries `new-2`.
    # A broken symlink planted there must not be followed by write_text.
    write_memory(tmp_path, "New", "first", now=_clock())
    outside = tmp_path / "PWNED.md"
    (tmp_path / ".agent-memory" / "new-2.md").symlink_to(outside)  # broken symlink
    with pytest.raises(ValueError):
        write_memory(tmp_path, "New", "second", now=_clock())
    assert not outside.exists()  # the real property: nothing written outside
```
...
def write_memory(
    ...
    resolve_memory_file(repo_path, final_slug)  # re-check the SUFFIXED name: a symlink
    # planted at `slug-2.md` would otherwise be followed by write_text and escape .agent-memory/
    fact = MemoryFact(
        slug=final_slug,
        title=title,
        kind=kind,
...
`resolve_memory_file` is called for `slug` before `mkdir` so a traversing title never creates the directory, **and again for `final_slug` after `_next_free`** — the collision suffix produces a *different* name than the base slug, and `Path.exists()` returns `False` for a broken symlink, so a symlink planted at `slug-2.md` would otherwise be followed by `write_text` and escape the directory. The second check resolves that symlink and raises. This is a vulnerability class beyond §5.1's `title`/`supersedes`: the *suffixed derivative* of an otherwise-safe title. `serialize_fact` raises on a newline title before `path.write_text`, so nothing lands on disk.
...
Expected: PASS (all write-path tests + the read tests). The suffix-symlink and
neutralized-traversal cases are the load-bearing ones — confirm they pass for the
right reason (the suffix-symlink test must FAIL if you drop the `final_slug` re-check).
...
def test_remember_surfaces_rejection_as_an_error(tmp_path):
    ...
    # it believes succeeded. Use a GENUINE rejection: a traversal *title* is not
    # one — slugify neutralizes "../escape" to the safe slug "escape" and the
    # write SUCCEEDS. `supersedes` naming no existing memory genuinely raises.
    remember = build_remember(tmp_path)
    with pytest.raises(ValueError):
...
> **Execution note (do Tasks 6 and 7 as ONE commit).** These two tasks are the
> two ends of a single producer/consumer contract: Task 6 removes `mcp_url`/`mcp_token`
> from `AgentProcess` (the consumer), and Task 7 stops `Supervisor` (the producer,
> `supervisor.py`) emitting them. Landing them separately leaves the suite red in
> between — `Supervisor` would call `agent_factory(mcp_url=…)` against a signature that
> dropped it (a `TypeError`, not a soft mismatch). Also, the caller list below
> undercounts: besides the two tests named in Task 6 Step 4, the kwarg change touches
> `tests/test_worker_app.py`, `tests/test_app.py`, and `tests/test_supervisor.py`
> (all assert `captured["mcp_url"]` — assertions only, their `FakeAgent`s take `**kwargs`),
> and `tests/integration/test_mailbox_e2e.py` (passes `mcp_url=` to a real `AgentProcess`;
> gated/skipped but must still compile). Before committing, run
> `grep -rn "mcp_url\|mcp_token" src/ tests/` and confirm zero LIVE kwarg references
> remain (a local `mcp_url = await shim.start()` is fine). This was executed as one
> merged commit.
