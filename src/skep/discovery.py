...
async def advertise(
    host: str, port: int, *, instance: str = "skep-queen", public_url: str | None = None
) -> AsyncServiceHandle:
    azc = AsyncZeroconf()
...
async def browse(timeout: float = 3.0) -> str | None:
    ...
    async def _resolve(type_: str, name: str) -> None:
        info = await azc.async_get_service_info(
            type_, name, timeout=int(timeout * 1000)
        )
        if info is None:
            return
...
async def resolve_queen_url(
    config: WorkerConfig, *, browse_timeout: float = 3.0
) -> str | None:
    if config.queen_url:
